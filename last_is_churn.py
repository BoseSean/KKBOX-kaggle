...
transactions = transaction.drop(columns=['payment_method_id', 'payment_plan_days',
                                         'plan_list_price', 'actual_amount_paid', 'is_auto_renew', 'is_cancel'])
transactions = transactions.sort_values(
    ['msno', 'transaction_date'])
result = dp.DataFrame(columns=['msno', 'last_1_is_churn', 'last_2_is_churn',
                               'last_3_is_churn', 'last_4_is_churn', 'last_5_is_churn', 'churn_rate', 'churn_count'])
...
for i, row in tqdm(transactions.iterrows(), total=total_rows):
    ...
    if (msno != prev_msno):
        calc_churn(transaction_dates, membership_expire_dates, prev_msno)
        transaction_dates = []
...
result.to_csv('last_is_churns.csv'.format(target), index=False)
